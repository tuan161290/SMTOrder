...
namespace PandaApp.SerialCommnunication
{
    public class SerialHelper
    {
        ...
        public SerialHelper(string Port, int TimeOut, int baudRate, string InstanceName)
        {
            try
            {
        ...
        public async Task<byte[]> ReadUCTAsync()
        {
            try
            {
                ...
                {
                    int TimeOut = 100;
                    var receiveTask = _serialPort.BaseStream.ReadAsync(buffer, 0, buffer.Length);
        ...
        public async Task<byte[]> ReadAsync()
        {
            try
            {
                ...
                if (_serialPort.IsOpen)
                {
                    var receiveTask = _serialPort.BaseStream.ReadAsync(buffer, 0, 64);
